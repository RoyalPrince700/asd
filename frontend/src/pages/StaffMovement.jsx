...
export function StaffMovement() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      await loadRecords();
      toast({
        message: editingId ? "Transaction updated." : "Transaction posted.",
        type: "success",
      });
    } catch (err) {
    ...
    <div className="page">
      <header className="page-head">
        ...
        <p className="lede tight">
          ...
          the transaction. Each entry appears in your records below and updates
          balances on the Inventory tab and CFO dashboard.
        </p>
      </header>
      ...
      <form className="entry-card" onSubmit={onSubmit}>
        ...
        <div className="grid-3">
          <label>
            ...
            >
              <option value="">
                {isTrifoneStaff ? "Select an item…" : "Select a book…"}
              </option>
              ...
              {editingId &&
              form.productName &&
              !products.some((product) => product.name === form.productName) ? (
        ...
        <div className="actions">
          <button type="submit" disabled={busy || closing < 0 || !form.productName}>
            {editingId ? "Save changes" : "Post transaction"}
          </button>
          {editingId ? (
